...
Now from your BASH terminal clone your fork(ed) repository. If you are
using HTTP to authenticate, you will do something like this:
...
For much of the bootcamp, you should type the example code snippets at
an interactive Python terminal. I recommend using either the IPython
shell or the IPython notebook. To start an IPython shell, type the
following at a BASH prompt:
...
Recall from last semester that variables are not their values in Python
(think “I am not my name, I am the person named XXX”). You can think of
variables as tags on objects. In particular, variables can be bound to
an object of one type and then reassigned to an object of another type
without error.
...
`pep8 <https://pypi.python.org/pypi/pep8>`__ package that provides a
commandline tool to check your code against some of the PEP8 standard
conventions. Similarly,
`autopep8 <https://pypi.python.org/pypi/autopep8>`__ provides a tool to
...
The last two links discuss the NumPy docstring [1]_ standard. Let’s
...
While we will be using Python 2 for this bootcamp, in the near future
...
[sec:datastructures]Data Structures
-----------------------------------
...
which you worked with in Statistics 243 (problem set 3).

I’ve provided a Python script (``ex3/munge.py``) that scraps the web,
processes the speeches, and saves the processed information to a JSON
file (``ex3/speeches.json``).

You should write a Python script [3]_ to:

#. Load the data from the JSON file.

#. Count the number of speeches (you may need to do some data cleaning).

#. Create a list with just the names of the presidents.

#. Create a list with just the dates of the speeches.

#. Write a function that takes a speech object a returns the tuple
   ``(<president>, <number of words>, <date>)``

#. Create a list of tuples by calling your function on all the speech
   objects.

#. Create a list of the speech dates sorted based on the length of
   speech.

For the last task you may want to take a look at the Python Sorting How
To:
